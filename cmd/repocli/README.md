...
From version >= 0.5.0, `repocli` also supports retry on failed file upload and download.  This retry feature is disabled by default and can be enabled for `put`, `get`, `mput` and `mget` operations with the `-r N` option where `N` is the number of retries.
